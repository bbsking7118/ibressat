...
from stocklab.kiwoom.config.log_class import *
...
class Kiwoom(QAxWidget):
    ...
    def login_slot(self, err_code):
        self.logging.logger.info(errors(err_code)[1])
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        if sRQName == "예수금상세현황요청":
            ...
            self.logging.logger.info("계좌평가잔고내역요청 싱글데이터 : %s - %s - %s" % (total_buy_money, total_profit_loss_money, total_profit_loss_rate))
            ...
            for i in range(rows):
                ...
                self.logging.logger.info("종목코드: %s - 종목명: %s - 보유수량: %s - 매입가:%s - 수익률: %s - 현재가: %s" % (
            ...
            self.logging.logger.info("sPreNext : %s" % sPrevNext)
            ...
            for i in range(rows):
                ...
                self.logging.logger.info("미체결 종목 : %s "  % self.not_account_stock_dict[order_no])
            ...
            self.logging.logger.info("남은 일자 수 %s" % cnt)
            ...
            if 0 : # sPrevNext == "2":
                ...
                self.logging.logger.info("총 일수 %s" % len(self.calcul_data))
    ...
    def calculator_fnc(self):
        ...
        self.logging.logger.info("코스닥 갯수 %s " % len(code_list))

        for idx, code in enumerate(code_list):
            self.dynamicCall("DisconnectRealData(QString)", self.screen_calculation_stock)  # 스크린 연결 끊기

            self.logging.logger.info("%s / %s :  KOSDAQ Stock Code : %s is updating... " % (idx + 1, len(code_list), code))
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간":
            ...
            if self.isTradingState != "OnTrading":
                return
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                self.logging.logger.debug("매도콜백진입 in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys()")
                ...
                if asd['매매가능수량'] > 0 and (meme_rate > 20 or meme_rate < -7):
                    ...
                    if order_success == 0:
                        self.logging.logger.info("매도주문 전달 성공")
                        del self.account_stock_dict[sCode]
                    else:
                        self.logging.logger.info("매도주문 전달 실패")

            elif sCode in self.jango_dict.keys():
                self.logging.logger.debug("매도콜백진입 in self.jango_dict.keys()")
                ...
                self.logging.logger.debug("매수콜백진입")
                if b > (j + self.portfolio_stock_dict[sCode]["매수기준"] * 0.5):
                #if b > (j + x * 0.5):
                    self.logging.logger.info("함수 : trdata_slot => 주식일봉차트조회 => OnTrading => 매수콜백진입")
                    self.logging.logger.info("매수조건 통과 %s (현재가)%s (j)%s (고-저)%s (기준)%s" % (
                        sCode, b, j, self.portfolio_stock_dict[sCode]["매수기준"],
                        j + self.portfolio_stock_dict[sCode]["매수기준"] * 0.5))
                    self.logging.logger.info("매수조건 통과 %s " % sCode)
                    result = (self.use_money * 0.1) / e
                    quantity = int(result)

                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["신규매수", self.portfolio_stock_dict[sCode]["주문용스크린번호"], self.account_num, 1, sCode, quantity, e, self.realType.SENDTYPE['거래구분']['시장가'], ""]
                    )

                    if order_success == 0:
                        self.logging.logger.info("매수주문 전달 성공")
                    else:
            ...
            #
            #         if order_success == 0:
            #             self.logging.logger.info("매수취소 전달 성공")
            #         else:
            #             self.logging.logger.info("매수취소 전달 실패")
            #
            #     elif not_quantity == 0:
    ...
    def make_ConditionFile(self):
        ...
        self.logging.logger.info("갯수 %s " % len(code_list))
        for idx, code in enumerate(code_list):
            self.dynamicCall("DisconnectRealData(QString)", self.screen_calculation_stock)  # 스크린 연결 끊기
    ...
    def sell_all(self):
        self.logging.logger.info("함수 : sell_all")
        asd = list(self.account_stock_dict)
        for sCode in asd:
            asd = self.account_stock_dict[sCode]
            #meme_rate = (b - asd['매입가']) / asd['매입가'] * 100
            if asd['매매가능수량'] > 0:
    ...
    def startSystemTrading(self):
        try:
            self.logging.logger.info("startSystemTrading 시작")
            # # self.account_num
            ...
            while True:
                ...
                if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
                    self.logging.logger.info("Today is Saturday or Sunday.")
                    exit()
                if t_9 < t_now < t_start and soldout == False:
                    self.logging.logger.info("AM 09:00 ~ AM 09:05!")
                    self.logging.logger.info("Account : %s 체결잔고 : %s 미체결잔고 : %s 포트폴리오 %s"
                ...
                if t_start < t_now < t_sell:  # AM 09:05 ~ PM 03:00 : 시스템트레이딩
                    # self.logging.logger.info("AM 09:05 ~ AM 03:00!")
                    # self.logging.logger.info("Account : %s 체결잔고 : %s 미체결잔고 : %s 포트폴리오 %s"
                    #                           % (len(self.account_stock_dict), len(self.jango_dict),
                    #                              len(self.not_account_stock_dict), len(self.portfolio_stock_dict)))
                    self.isTradingState = "OnTrading"
                ...
                if t_exit < t_now:  # PM 03:20 ~ :프로그램 종료
                    self.logging.logger.info("PM 03:20 ~ :프로그램 종료!")
                    if 0 :
            ...
            self.logging.logger.info('`main -> exception! ' + str(ex) + '`')
